...
const numerologyStones: Record<number, { stones: string[]; meaning: string; description: string }> = {
  ...
  3: {
    ...
    description: "Число три — это энергия творца, художника, живого существа, рождённого для самовыражения. Вам важно находить каналы для реализации своих идей и делиться внутренним светом с миром. Аметист помогает сосредоточиться и настроиться на творческий поток, голубой топаз открывает горловую чакру для свободного самовыражения, а флюорит упорядочивает хаотичные мысли и направляет вдохновение в конкретные действия.",
  },
  4: {
    stones: ["Чёрный турмалин", "Красная яшма", "Гематит"],
    meaning: "Стабильность, порядок, фундамент",
    description: "Число четыре — это энергия строителя, индивида, который создаёт прочный фундамент для себя и других. Вы цените порядок, надёжность и системный подход к жизни. Чёрный турмалин заземляет и защищает от внешнего хаоса, красная яшма укрепляет связь с землёй и даёт выносливость, а гематит придаёт дисциплину и помогает доводить дела до конца.",
  },
  5: {
    stones: ["Аквамарин", "Ларимар", "Хризоколла"],
    meaning: "Свобода, перемены, приключения",
    description: "Число пять — это энергия путешественника, искателя, живого существа, жаждущего перемен и новых горизонтов. Вам тесно в рамках и рутине, вы стремитесь к свободе и разнообразию опыта. Аквамарин очищает эмоциональное поле и помогает адаптироваться к переменам, ларимар успокаивает страхи перед неизвестным, а хризоколла помогает находить мудрость в каждом новом опыте.",
  },
  6: {
    stones: ["Родонит", "Нефрит", "Розовая шпинель"],
    meaning: "Любовь, ответственность, забота",
    description: "Число шесть — это энергия целителя, хранителя семьи, индивида, который заботится о других. Вы обладаете глубоким чувством ответственности и стремитесь создать вокруг себя атмосферу любви и безопасности. Родонит исцеляет эмоциональные раны и восстанавливает способность любить, нефрит привлекает гармонию в семейные отношения, а розовая шпинель помогает сбалансировать заботу о других с заботой о себе.",
  },
  7: {
  ...
  8: {
    ...
    description: "Число восемь — это энергия воина, индивида власти и реализации. Вы обладаете мощным потенциалом для достижения материальных и духовных целей. Тигровый глаз укрепляет уверенность в себе и помогает принимать смелые решения, пирит притягивает финансовое изобилие и активирует волевой центр, а хризоберилл усиливает самодисциплину и помогает превращать планы в реальность.",
  },
  9: {
    stones: ["Молдавит", "Танзанит", "Празиолит"],
    meaning: "Завершение, мудрость, служение",
    description: "Число девять — это энергия завершающего цикла, мудреца, живого существа, призванного служить высшей цели. Вы обладаете глубоким состраданием и способностью видеть общую картину. Молдавит ускоряет духовную эволюцию и помогает отпустить старое, танзанит соединяет сердечную и коронную чакры для духовного прозрения, а празиолит помогает интегрировать духовный опыт в повседневную жизнь.",
  },
};
