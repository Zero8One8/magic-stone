...
const articles = [
  ...
  },
  {
    ...
    content: [
      { type: "p" as const, text: "Вы когда-нибудь просыпались с тяжестью в груди, хотя ничего особенного не произошло? Или чувствовали, как сжимается горло перед разговором с тем, кто когда-то вас ранил? Или замечали, как болит спина именно в те дни, когда вспоминаете старую обиду? Это не совпадение." },
      { type: "p" as const, text: "Тело может говорить о том, что внутри застряло непереваренное, непрощенное, не отпущенное. Обиды, злость, зависть, ненависть, страх — все это может восприниматься как тяжелые эмоциональные сгустки, влияющие на качество жизни и самочувствие." },
      { type: "p" as const, text: "Пока человек не встречается с этой болью лицом к лицу, не признает ее и не проживает, внутреннее напряжение может накапливаться и проявляться в повторяющихся сценариях." },

      { type: "h2" as const, text: "Обида: это яд, который вы пьете сами" },
      { type: "p" as const, text: "Обида часто рождается там, где ждали любви, принятия и поддержки, а получили холод, предательство или обман. Она оседает тяжестью в груди, комом в горле, закрытостью сердца." },
      { type: "p" as const, text: "Где может проявляться: сердце и грудная клетка, дыхание, зажимы в плечах и верхней части спины." },
      { type: "p" as const, text: "Как помогают камни: розовый кварц — для мягкого раскрытия сердца, родонит — для эмоционального исцеления, намоленные камни — для более глубокой практики прощения и освобождения." },

      { type: "h2" as const, text: "Злость: огонь, который сжигает изнутри" },
      { type: "p" as const, text: "Злость — это энергия действия. Если ей не дать безопасный выход, она остается внутри как хроническое внутреннее напряжение." },
      { type: "p" as const, text: "Где может проявляться: тяжесть в животе, раздражительность, зажимы в челюсти, ощущение постоянного внутреннего давления." },
      { type: "p" as const, text: "Как помогают камни: тигровый глаз — трансформация злости в действие, цитрин — возвращение ясности и уверенности, янтарь — мягкое очищение от эмоциональной токсичности." },

      { type: "h2" as const, text: "Зависть: яд сравнения" },
      { type: "p" as const, text: "Зависть возникает, когда человек теряет контакт с собственной ценностью и живет через сравнение. Это истощает, усиливает тревогу и мешает видеть свои достижения." },
      { type: "p" as const, text: "Как помогают камни: зеленый авантюрин — благодарность и самопринятие, малахит — глубокая трансформация застарелых эмоциональных паттернов." },

      { type: "h2" as const, text: "Ненависть: самый тяжелый груз" },
      { type: "p" as const, text: "Ненависть — это боль, зафиксированная в крайней форме. Она отнимает много сил, закрывает сердце и может разрушать изнутри." },
      { type: "p" as const, text: "Как помогают камни: черный турмалин — очищение и защита, обсидиан — честная встреча с тенью, аметист — духовное исцеление и внутреннее освобождение." },

      { type: "h2" as const, text: "Страх: паралич жизни" },
      { type: "p" as const, text: "Страх может превращать жизнь в режим выживания: решения даются трудно, тело постоянно напряжено, опора будто теряется." },
      { type: "p" as const, text: "Как помогают камни: красная яшма — заземление и устойчивость, гематит — внутренний стержень, бирюза — смелость говорить правду и выражать себя." },

      { type: "h2" as const, text: "Намоленные камни: почему они работают иначе" },
      { type: "p" as const, text: "Намоленные камни проходят через длительную практику молитвы и медитации. Их выбирают, когда нужна более глубокая поддержка в работе с тяжелыми состояниями." },

      { type: "h2" as const, text: "Как отпустить по-настоящему" },
      { type: "p" as const, text: "Носите камень, медитируйте с ним, позволяйте себе проживать чувства: плакать, злиться, признавать боль. Важна не борьба с эмоцией, а честная встреча с ней." },
      { type: "p" as const, text: "Ваше тело помнит все. Но оно также способно исцеляться, когда вы даете себе шанс на глубокую внутреннюю работу." },
    ],
  },
];
