...
function buildIntegratedSummary(results: SystemResult[]): string {
  ...
  const archetype = `${results[0].resultName} + ${results[2].resultName} + ${results[7].resultName}`;
  const style = firstSentence(results[1]?.brief ?? "");
  const mission = firstSentence(results[3]?.brief ?? "");
  return [
    `Ваш базовый архетип: ${archetype}. Это сочетание даёт не линейный характер, а многослойный профиль с высокой глубиной восприятия и сильным внутренним центром.`,
    `Ключевой стиль проявления: ${style}`,
    `Линия предназначения: ${mission}`,
    `Сильная сторона портрета — способность одновременно держать стратегию и тонкую интуицию. Риск-зона — перегруз от контроля, когда скорость жизни начинает конфликтовать с внутренним ритмом.`,
    `Главный камень-компилятор этого портрета — ${topCrystal}: он связывает разные уровни вашей системы в единый вектор, чтобы решения рождались из ясности, а не из тревоги.`,
  ].join(" ");
}
...
function buildFullPreview(results: SystemResult[], form: FormData): FullPreviewSection[] {
  ...
  return [
    {
      title: "Архетипическая Доминанта",
      text: `Итог не сводится к отдельным прогнозам: системы сходятся в единую формулу личности. Здесь соединяются воля, интуиция и способность к глубокому обновлению. Такой профиль раскрывается максимально ярко, когда вы выбираете длинную стратегию и действуете из внутренней опоры, а не из внешнего давления.`,
    },
    {
      title: "Сила И Риск В Одной Карте",
      text: `Сильные стороны: системное мышление, тонкое считывание контекста, устойчивость в сложных циклах. Зона напряжения: склонность брать сверхконтроль и перерасходовать ресурс там, где нужна дозированная точность. Практическая формула баланса: стратегия через сердце, а не через внутренний прессинг.`,
    },
    {
      title: "Стратегия Реализации",
      text: `Ваш путь раскрывается через три опоры: 1) ясный приоритет на квартал, 2) регулярное восстановление поля, 3) выбор среды, где ценится глубина, а не суета. Когда эти три условия соблюдены, жизненные решения становятся точными и экономят годы пути.`,
    },
    {
      title: "Камни Интеграции",
      text: `В полном раскладе камни назначаются по уровням задачи: ядро личности (${topCrystal}), стиль проявления (${styleCrystal}), реализация предназначения (${purposeCrystal}), защитный контур (${protectionCrystal}). Это даёт не «красивый набор», а рабочую систему кристаллов под ваш текущий цикл.${form.city ? ` География ${form.city} добавляет точность в астрологической части карты.` : ""}`,
    },
    {
      title: "Что Вы Получаете В Полной Версии",
      text: `Пошаговую дорожную карту: какие качества усиливать первыми, какие сценарии закрывать, какие решения дадут максимальный прирост в деньгах, отношениях и личной силе. Результат — целостный документ, который можно использовать как персональный навигатор на 6–12 месяцев.`,
    },
  ];
}
